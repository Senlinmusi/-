...
export default function YX1() {
  ...
  useEffect(() => {
    ...
    const i = ZY1('/q.png'), i2 = ZY1('/q2.png'), j = ZY1('/p.png'), 
          k = ZY1('/C1.png'), l = ZY1('/z.png'), r1 = ZY1('/return.png'), 
          m1 = ZY1('/musicOn.png'), m2 = ZY1('/musicOff.png')
    ...
    const YX2 = () => {
      ...
      HH1(l, 180, 350, 0.12)
      HH1(k, p2x, 100, 0.18, 0, FX1)
      HH1(j, p2x + (40 * FX1), p2y - 20, 0.15)
      HH1(QP1 ? i : i2, qx, qy, 0.05, qr)
      HH1(j, p1x, p1y, 0.15)

      v.strokeStyle = 'black'
      v.lineWidth = 2
      v.beginPath()
      v.arc(35, 35, 14, 0, Math.PI * 2)
      v.stroke()
      HH1(ON1 ? m1 : m2, 35, 35, 0.02)

      v.beginPath()
      v.arc(35, 75, 14, 0, Math.PI * 2)
      v.stroke()
      ...
      v.textAlign = 'right'
      v.font = '36px Zpix'
      v.strokeText(`${fsc}`, 340, 50)
      v.fillText(`${fsc}`, 340, 50)
    ...
    const YD1 = (e: MouseEvent | TouchEvent) => {
      ...
      const { x, y } = ZB1('touches' in e ? e.touches[0].clientX : e.clientX, 'touches' in e ? e.touches[0].clientY : e.clientY)
      WS1 = x - ZW1
      p1x = Math.max(0, Math.min(360, x))
      p1y = Math.max(480, Math.min(640, y))
      ZW1 = p1x
    }

    cv.addEventListener('mousedown', CZ1)
    window.addEventListener('mousemove', YD1)
    window.addEventListener('mouseup', () => MG1 = false)
    cv.addEventListener('touchstart', CZ1, { passive: false })
    cv.addEventListener('touchmove', YD1, { passive: false })
    cv.addEventListener('touchend', () => MG1 = false)

    return () => {
      window.removeEventListener('mousemove', YD1)
      window.removeEventListener('mouseup', () => MG1 = false)
      yy.pause()
    }
  }, [])
